...
export const registerUser = async (req: Request, res: Response) => {
    try {
        ...
        if (error instanceof ZodError) {
            ...
            res.status(400).json({ message: 'Invalid Inputs', error: error.errors.map((error) => error.message) });
        }
        else {
            console.error(error);
...
export const loginUser = async (req: Request, res: Response) => {
    try {
        ...
        if (error instanceof ZodError) {
            ...
            res.status(500).json({ message: 'Error during login', error: 'Intrenal Server Error' });
        }
        return;
    }
};
